...
pub struct PDLwSlackStatement<P: ECPoint> {
...
#[derive(Clone)]
pub struct PDLwSlackWitness<S: ECScalar> {
    pub x: S,
    pub r: BigInt,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PDLwSlackProof<P> {
    z: BigInt,
    u1: P,
    u2: BigInt,
    u3: BigInt,
...
impl<P> PDLwSlackProof<P>
where
    P: ECPoint + Clone,
{
    pub fn prove(witness: &PDLwSlackWitness<P::Scalar>, statement: &PDLwSlackStatement<P>) -> Self {
        let q3 = P::Scalar::q().pow(3);
        let q_N_tilde = P::Scalar::q() * &statement.N_tilde;
        ...
        let u1 = statement.G.clone() * <P::Scalar as ECScalar>::from(&alpha);
    ...
    pub fn verify(&self, statement: &PDLwSlackStatement<P>) -> FsDkrResult<()> {
        let e = HSha256::create_hash(&[
            &statement.G.bytes_compressed_to_big_int(),
            &statement.Q.bytes_compressed_to_big_int(),
            &statement.ciphertext,
        ...
        let e_fe_neg = ECScalar::from(&(P::Scalar::q() - &e));
