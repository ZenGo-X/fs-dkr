...
struct BobZkpRound1<S: ECScalar> {
    pub alpha: BigInt,
    pub beta: BigInt,
    pub gamma: BigInt,
    pub ro: BigInt,
...
impl<S: ECScalar> Zeroize for BobZkpRound1<S> {
    fn zeroize(&mut self) {
        self.alpha.zeroize();
        self.beta.zeroize();
        self.gamma.zeroize();
...
impl<S: ECScalar> Drop for BobZkpRound1<S> {
    fn drop(&mut self) {
        self.zeroize();
    }
}

impl<S: ECScalar> BobZkpRound1<S> {
    /// `b` - Bob's secret
...
struct BobZkpRound2<S: ECScalar> {
    pub s: BigInt,
    pub s1: BigInt,
    pub s2: BigInt,
    pub t1: BigInt,
...
impl<S: ECScalar> BobZkpRound2<S> {
    ...
    P::Scalar: Clone,
{
    ...
    pub fn generate(
        ...
        let e = if check {
            let (X, u) = {
                let ec_gen: P = ECPoint::generator();
                let alpha: P::Scalar = ECScalar::from(&round1.alpha);
...
#[derive(Clone, PartialEq, Debug, Serialize, Deserialize)]
pub struct BobProofExt<P: ECPoint> {
    proof: BobProof<P>,
    u: P,
}

#[allow(clippy::too_many_arguments)]
impl<P: ECPoint> BobProofExt<P>
where
    P: ECPoint + Clone,
    P::Scalar: Clone,
{
    pub fn verify(
        ...
        let (x1, x2) = {
            let ec_gen: P = ECPoint::generator();
            let s1: P::Scalar = ECScalar::from(&self.proof.s1);
            let e: P::Scalar = ECScalar::from(&self.proof.e);
...
pub(crate) mod tests {
    ...
    fn alice_zkp() {
        ...
        let alice_proof = AliceProof::<GE>::generate(&a, &cipher, &ek, &dlog_statement, &r);
